"""Learn which pitch classes added by the frozen decoder should survive.

Training is restricted to decoder-generated candidate pitch classes.  Original
foundation notes are assigned probability one and missing classes probability
zero, so this experiment can only retract a high-confidence false positive.
"""
...
from .fit_pianist_pitch_class_addition_ranker import estimator
...
def song_arrays(song: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "labels": np.asarray([row["label"] for row in examples], dtype=np.int8).reshape(cells, 12),
        "candidate": np.asarray([row["candidate"] for row in examples], dtype=np.int8).reshape(cells, 12),
        "source": np.asarray([row["source"] for row in examples], dtype=np.int8).reshape(cells, 12),
        "generated": np.asarray([row["decoderGenerated"] for row in examples], dtype=bool).reshape(cells, 12),
    }


def generated_mask(arrays: dict[str, Any]) -> np.ndarray:
    return arrays["generated"].reshape(-1)


def probability_grid(arrays: dict[str, Any], model: Any) -> np.ndarray:
    candidate = arrays["candidate"].reshape(-1)
    generated = generated_mask(arrays)
    probabilities = np.zeros(candidate.shape[0], dtype=float)
    probabilities[(candidate > 0) & ~generated] = 1.0
    if np.any(generated):
        probabilities[generated] = model.predict_proba(arrays["features"][generated])[:, 1]
...
def main() -> None:
    ...
    parser.add_argument("--remove-threshold", type=float, default=0.40)
    parser.add_argument(
        "--estimator",
    ...
    if not 0.0 <= args.remove_threshold < 1.0:
        raise ValueError("Remove threshold must be between zero and one")
    ...
    for held_out in song_ids:
        ...
        train_features = np.concatenate(
            [arrays[song_id]["features"][generated_mask(arrays[song_id])] for song_id in training_ids]
        )
        train_labels = np.concatenate(
            [arrays[song_id]["labels"].reshape(-1)[generated_mask(arrays[song_id])] for song_id in training_ids]
        )
        if len(np.unique(train_labels)) < 2:
            raise ValueError("Generated-note labels need both keep and remove examples")
        model = estimator(str(args.estimator))
        model.fit(train_features, train_labels)
        probabilities = probability_grid(arrays[held_out], model)
        ...
        scores = score_payload(
            held_out,
            probabilities,
            model_id=f"decoder-generated-auditor-loso-{stem}",
            model_sha256=model_sha,
            training_song_ids=training_ids,
            feature_names=FEATURE_NAMES,
        )
        scores["decisionScope"] = "decoder-generated-removals-only"
        atomic_json(scores_path, scores)
        baseline = set_metrics(
            arrays[held_out], probabilities, add_threshold=2.0, remove_threshold=-1.0,
            source_supported_additions_only=True,
        )
        candidate = set_metrics(
            arrays[held_out], probabilities, add_threshold=2.0,
            remove_threshold=float(args.remove_threshold), source_supported_additions_only=True,
        )
        generated_count = int(np.sum(arrays[held_out]["generated"]))
        folds.append(
            {
                ...
                "heldOutGeneratedDecisions": generated_count,
    ...
    all_features = np.concatenate(
        [arrays[song_id]["features"][generated_mask(arrays[song_id])] for song_id in song_ids]
    )
    all_labels = np.concatenate(
        [arrays[song_id]["labels"].reshape(-1)[generated_mask(arrays[song_id])] for song_id in song_ids]
    )
    ...
    policy = {
        "schema": "polymath-decoder-generated-pitch-class-auditor-policy-v1",
        "id": f"decoder-generated-auditor-{args.estimator}-origin274-opened-v001",
        "production": False,
        "commercialUseAllowed": False,
        "decisionScope": "decoder-generated-removals-only",
        "featureContract": "origin274",
        "addThreshold": 1.0,
        "removeThreshold": float(args.remove_threshold),
        "sourceSupportedAdditionsOnly": True,
        "removeGeneratedOnly": True,
        "preserveMelodyRemovals": True,
        "modelSha256": model_sha,
        "warning": "Opened development only; a new untouched transfer is mandatory.",
    }
    atomic_json(output_dir / "policy.json", policy)
    deltas = [float(row["deltas"]["f1"]) for row in folds]
    report = {
        "schema": "polymath-decoder-generated-pitch-class-auditor-training-v1",
        ...
        "negativeDecisions": int(len(all_labels) - np.sum(all_labels)),
        "policy": policy,
        "folds": folds,
        "aggregate": {
            "averageF1Delta": round(float(np.mean(deltas)), 6),
            "worstF1Delta": round(min(deltas), 6),
            "totalRemovals": sum(int(row["candidate"]["removals"]) for row in folds),
        },
        "featureImportance": feature_importance(final_model),
        "gates": {
            "allSongsCellPitchClassF1NonRegression": all(value >= 0.0 for value in deltas),
    ...
    print(json.dumps({"report": str(report_path), "aggregate": report["aggregate"], "gates": report["gates"], "examples": {"positive": report["positiveDecisions"], "negative": report["negativeDecisions"]}}, indent=2))
