"""Fit a whole-song-held-out ranker for source-supported chord additions.

Unlike the general chord-membership decoder, this model is trained only on
pitch classes that are absent from the arranged gesture but present in the
local transcription.  Existing candidate notes receive probability one and
are therefore never removed by this experiment.
"""

from __future__ import annotations

import argparse
import json
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

import joblib
import numpy as np
from sklearn.ensemble import ExtraTreesClassifier, RandomForestClassifier

from .fit_pianist_pitch_class_boosted_decoder import (
    atomic_json,
    load_json,
    safe_name,
    score_payload,
    set_metrics,
    sha256_file,
    song_arrays,
)
from .fit_pianist_pitch_class_decoder import FEATURE_NAMES as BASE_FEATURE_NAMES
from .pianist_pitch_class_context import (
    FEATURE_NAMES as CONTEXT_FEATURE_NAMES,
    context_examples_from_song,
)
from .pianist_pitch_class_evidence_context import (
    FEATURE_NAMES as EVIDENCE_FEATURE_NAMES,
    evidence_examples_from_song,
)
from .pianist_pitch_class_sequence_context import (
    FEATURE_NAMES as SEQUENCE_FEATURE_NAMES,
    sequence_examples_from_song,
)


def estimator(
    kind: str,
    *,
    n_estimators: int | None = None,
    max_depth: int | None = None,
    min_samples_leaf: int = 8,
    max_features: str | float = "sqrt",
) -> ExtraTreesClassifier | RandomForestClassifier:
    common = {
        "n_estimators": n_estimators or (60 if kind == "random-forest" else 50),
        "max_depth": max_depth or (8 if kind == "random-forest" else 10),
        "min_samples_leaf": min_samples_leaf,
        "max_features": max_features,
        "n_jobs": -1,
    }
    if kind == "random-forest":
        return RandomForestClassifier(
            **common,
            class_weight="balanced_subsample",
            random_state=41,
        )
    if kind == "extra-trees":
        return ExtraTreesClassifier(
            **common,
            class_weight="balanced",
            random_state=23,
        )
    raise ValueError(f"Unknown estimator: {kind}")


def context_song_arrays(song: dict[str, Any]) -> dict[str, Any]:
    return examples_to_arrays(song, context_examples_from_song(song))


def evidence_song_arrays(song: dict[str, Any]) -> dict[str, Any]:
    return examples_to_arrays(song, evidence_examples_from_song(song))


def sequence_song_arrays(song: dict[str, Any]) -> dict[str, Any]:
    return examples_to_arrays(song, sequence_examples_from_song(song))


def examples_to_arrays(
    song: dict[str, Any], examples: list[dict[str, Any]]
) -> dict[str, Any]:
    if not examples or len(examples) % 12:
        raise ValueError(f"Song {song.get('id')!r} has an invalid example grid")
    cells = len(examples) // 12
    return {
        "examples": examples,
        "features": np.asarray(
            [row["features"] for row in examples], dtype=np.float32
        ),
        "labels": np.asarray(
            [row["label"] for row in examples], dtype=np.int8
        ).reshape(cells, 12),
        "candidate": np.asarray(
            [row["candidate"] for row in examples], dtype=np.int8
        ).reshape(cells, 12),
        "source": np.asarray(
            [row["source"] for row in examples], dtype=np.int8
        ).reshape(cells, 12),
    }


def eligible_mask(arrays: dict[str, Any]) -> np.ndarray:
    candidate = arrays["candidate"].reshape(-1)
    source = arrays["source"].reshape(-1)
    return (candidate <= 0) & (source > 0)


def probability_grid(
    arrays: dict[str, Any],
    model: ExtraTreesClassifier | RandomForestClassifier,
) -> np.ndarray:
    shape = arrays["labels"].shape
    candidate = arrays["candidate"].reshape(-1)
    mask = eligible_mask(arrays)
    probabilities = np.zeros(candidate.shape[0], dtype=float)
    probabilities[candidate > 0] = 1.0
    if np.any(mask):
        probabilities[mask] = model.predict_proba(arrays["features"][mask])[:, 1]
    return probabilities.reshape(shape)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--audit", action="append", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--add-threshold", type=float, default=0.675)
    parser.add_argument(
        "--estimator",
        choices=("extra-trees", "random-forest"),
        default="extra-trees",
    )
    parser.add_argument("--context-features", action="store_true")
    parser.add_argument("--evidence-features", action="store_true")
    parser.add_argument("--sequence-features", action="store_true")
    parser.add_argument("--n-estimators", type=int)
    parser.add_argument("--max-depth", type=int)
    parser.add_argument("--min-samples-leaf", type=int, default=8)
    parser.add_argument("--max-features", default="sqrt")
    args = parser.parse_args()
    if not 0.0 <= args.add_threshold <= 1.0:
        raise ValueError("add threshold must be between zero and one")

    songs: list[dict[str, Any]] = []
    seen: set[str] = set()
    audit_inputs: list[dict[str, str]] = []
    for unresolved in args.audit:
        path = unresolved.resolve()
        audit_inputs.append({"path": str(path), "sha256": sha256_file(path)})
        for song in load_json(path).get("songs") or []:
            song_id = str(song.get("id") or "")
            if not song_id or song_id in seen:
                raise ValueError(f"Missing or duplicate song id: {song_id!r}")
            seen.add(song_id)
            songs.append(song)
    if len(songs) < 3:
        raise ValueError("At least three whole songs are required")

    if sum((args.context_features, args.evidence_features, args.sequence_features)) > 1:
        raise ValueError("Choose only one extended feature contract")
    try:
        max_features: str | float = float(args.max_features)
    except ValueError:
        max_features = str(args.max_features)
    if isinstance(max_features, float) and not 0.0 < max_features <= 1.0:
        raise ValueError("numeric max-features must be between zero and one")
    output_dir = args.output_dir.resolve()
    if args.sequence_features:
        array_builder = sequence_song_arrays
        feature_names = SEQUENCE_FEATURE_NAMES
        feature_contract = f"sequence{len(feature_names)}"
    elif args.evidence_features:
        array_builder = evidence_song_arrays
        feature_names = EVIDENCE_FEATURE_NAMES
        feature_contract = "evidence259"
    elif args.context_features:
        array_builder = context_song_arrays
        feature_names = CONTEXT_FEATURE_NAMES
        feature_contract = "context237"
    else:
        array_builder = song_arrays
        feature_names = BASE_FEATURE_NAMES
        feature_contract = "base93"
    arrays = {str(song["id"]): array_builder(song) for song in songs}
    song_ids = list(arrays)
    folds: list[dict[str, Any]] = []
    for held_out in song_ids:
        training_ids = [song_id for song_id in song_ids if song_id != held_out]
        train_features = np.concatenate(
            [
                arrays[song_id]["features"][eligible_mask(arrays[song_id])]
                for song_id in training_ids
            ]
        )
        train_labels = np.concatenate(
            [
                arrays[song_id]["labels"].reshape(-1)[
                    eligible_mask(arrays[song_id])
                ]
                for song_id in training_ids
            ]
        )
        model = estimator(
            str(args.estimator),
            n_estimators=args.n_estimators,
            max_depth=args.max_depth,
            min_samples_leaf=max(1, int(args.min_samples_leaf)),
            max_features=max_features,
        )
        model.fit(train_features, train_labels)
        probabilities = probability_grid(arrays[held_out], model)

        stem = safe_name(held_out)
        model_path = output_dir / "folds" / stem / "model.joblib"
        model_path.parent.mkdir(parents=True, exist_ok=True)
        temporary = model_path.with_name(model_path.name + ".tmp")
        joblib.dump(model, temporary)
        temporary.replace(model_path)
        model_sha = sha256_file(model_path)
        scores_path = output_dir / "scores" / f"{stem}.json"
        atomic_json(
            scores_path,
            score_payload(
                held_out,
                probabilities,
                model_id=f"addition-ranker-loso-{stem}",
                model_sha256=model_sha,
                training_song_ids=training_ids,
                feature_names=feature_names,
            ),
        )
        baseline = set_metrics(
            arrays[held_out],
            probabilities,
            add_threshold=2.0,
            remove_threshold=-1.0,
            source_supported_additions_only=True,
        )
        candidate = set_metrics(
            arrays[held_out],
            probabilities,
            add_threshold=float(args.add_threshold),
            remove_threshold=0.0,
            source_supported_additions_only=True,
        )
        folds.append(
            {
                "heldOutSong": held_out,
                "trainingSongIds": training_ids,
                "eligibleTrainingDecisions": int(train_labels.shape[0]),
                "model": {"path": str(model_path), "sha256": model_sha},
                "scores": {
                    "path": str(scores_path),
                    "sha256": sha256_file(scores_path),
                },
                "baseline": baseline,
                "candidate": candidate,
                "deltas": {
                    "f1": round(candidate["f1"] - baseline["f1"], 6),
                    "exactSetRate": round(
                        candidate["exactSetRate"] - baseline["exactSetRate"],
                        6,
                    ),
                },
            }
        )

    all_features = np.concatenate(
        [
            arrays[song_id]["features"][eligible_mask(arrays[song_id])]
            for song_id in song_ids
        ]
    )
    all_labels = np.concatenate(
        [
            arrays[song_id]["labels"].reshape(-1)[eligible_mask(arrays[song_id])]
            for song_id in song_ids
        ]
    )
    final_model = estimator(
        str(args.estimator),
        n_estimators=args.n_estimators,
        max_depth=args.max_depth,
        min_samples_leaf=max(1, int(args.min_samples_leaf)),
        max_features=max_features,
    )
    final_model.fit(all_features, all_labels)
    final_path = output_dir / "final-opened-development-model.joblib"
    final_path.parent.mkdir(parents=True, exist_ok=True)
    temporary = final_path.with_name(final_path.name + ".tmp")
    joblib.dump(final_model, temporary)
    temporary.replace(final_path)

    policy = {
        "schema": "polymath-pianist-addition-ranker-policy-v1",
        "id": (
            f"source-supported-addition-ranker-{args.estimator}-"
            f"{feature_contract}-opened-v001"
        ),
        "production": False,
        "commercialUseAllowed": False,
        "addThreshold": float(args.add_threshold),
        "removeThreshold": 0.0,
        "sourceSupportedAdditionsOnly": True,
        "registerStrategy": "nearest-candidate",
        "additionTiming": "candidate",
        "modelSha256": sha256_file(final_path),
        "warning": "Opened development only; a new untouched transfer is mandatory.",
    }
    atomic_json(output_dir / "policy.json", policy)
    f1_deltas = [float(row["deltas"]["f1"]) for row in folds]
    report = {
        "schema": "polymath-pianist-addition-ranker-training-v1",
        "createdAt": datetime.now(timezone.utc).isoformat(),
        "evidenceBoundary": "Whole-song leave-one-out opened development.",
        "audits": audit_inputs,
        "songs": song_ids,
        "estimator": str(args.estimator),
        "featureContract": feature_contract,
        "featureCount": len(feature_names),
        "hyperparameters": {
            "nEstimators": args.n_estimators,
            "maxDepth": args.max_depth,
            "minSamplesLeaf": max(1, int(args.min_samples_leaf)),
            "maxFeatures": max_features,
        },
        "eligibleDecisions": int(all_labels.shape[0]),
        "positiveDecisions": int(np.sum(all_labels)),
        "policy": policy,
        "folds": folds,
        "aggregate": {
            "averageF1Delta": round(float(np.mean(f1_deltas)), 6),
            "worstF1Delta": round(min(f1_deltas), 6),
            "totalAcceptedAdditions": sum(
                int(row["candidate"]["additions"]) for row in folds
            ),
        },
        "gates": {
            "allSongsCellPitchClassF1NonRegression": all(
                value >= 0.0 for value in f1_deltas
            ),
            "playableNoteGateStillRequired": True,
            "untouchedTransferStillRequired": True,
        },
        "decision": "RESEARCH_ONLY_NOT_PROMOTION_SAFE",
        "finalModel": {
            "path": str(final_path),
            "sha256": sha256_file(final_path),
            "trainingSongIds": song_ids,
        },
    }
    report_path = output_dir / "report.json"
    atomic_json(report_path, report)
    print(
        json.dumps(
            {
                "report": str(report_path),
                "aggregate": report["aggregate"],
                "gates": report["gates"],
                "decision": report["decision"],
            },
            indent=2,
        )
    )


if __name__ == "__main__":
    main()
