...
resource "aws_lb_listener" "ohio_https" {
  provider          = aws.ohio
  load_balancer_arn = aws_lb.ohio.arn
  port              = 443
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-TLS13-1-2-2021-06"
  certificate_arn   = aws_acm_certificate_validation.ohio_origin.certificate_arn
...
resource "aws_lb_listener" "singapore_https" {
  provider          = aws.singapore
  load_balancer_arn = aws_lb.singapore.arn
  port              = 443
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-TLS13-1-2-2021-06"
  certificate_arn   = aws_acm_certificate_validation.singapore_origin.certificate_arn
