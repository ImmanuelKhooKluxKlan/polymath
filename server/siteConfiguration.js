...
const NAVIGATION_REGISTRY = Object.freeze([
  ...
  Object.freeze({ id: 'find-teacher', path: '#find-teacher', defaultLabel: 'Learn', defaultGroup: 'more', defaultOrder: 70, access: 'public' }),
...
function publicSiteConfiguration(db) {
  ...
  return {
    ...
    navigation: configuration.navigation.map((item) => {
      ...
      return {
        ...item,
        ...(item.id === 'find-teacher' ? { label: 'Learn', visible: true } : {}),
        ...(['band', 'create-music'].includes(item.id) ? { visible: false } : {}),
