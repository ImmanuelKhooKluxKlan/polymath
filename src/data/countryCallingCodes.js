const RAW_COUNTRY_CODES = [
  ['AF', '+93'], ['AX', '+358 18'], ['AL', '+355'], ['DZ', '+213'], ['AS', '+1 684'],
  ['AD', '+376'], ['AO', '+244'], ['AI', '+1 264'], ['AQ', '+672'], ['AG', '+1 268'],
  ...
  ['MK', '+389'], ['MP', '+1 670'], ['NO', '+47'], ['OM', '+968'], ['PK', '+92'],
  ...
  ['SB', '+677'], ['SO', '+252'], ['ZA', '+27'], ['GS', '+500'], ['SS', '+211'],
...
function flagEmoji(iso) {
  ...
  return String.fromCodePoint(...[...iso].map((character) => 127397 + character.charCodeAt(0)));
}

export const COUNTRY_CALLING_CODES = Object.freeze(RAW_COUNTRY_CODES
  .map(([iso, dialCode]) => Object.freeze({
    iso,
...
export function countryByIso(iso = 'SG') {
  return COUNTRY_CALLING_CODES.find((country) => country.iso === String(iso).toUpperCase())
    || COUNTRY_CALLING_CODES.find((country) => country.iso === 'SG');
}

export function suggestedCountryIso(language = '') {
  const candidate = String(language || '').split(/[-_]/)[1]?.toUpperCase();
  return COUNTRY_CALLING_CODES.some((country) => country.iso === candidate) ? candidate : 'SG';
}
