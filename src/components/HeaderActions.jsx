export default function HeaderActions({ user, onNavigate, route }) {
  ...
  return (
    <div className={`global-actions ${isLesson ? 'lesson-actions' : ''}`}>
      ...
      <button className="buy-pro-button compact-pro" type="button" onClick={() => user?.pro ? onNavigate('account') : onNavigate('payment', { productId: 'polymath-pro' })}>
        <span>{user?.pro ? 'Pro' : 'Upgrade'}</span>
        <small>{user?.pro ? `${user.translationAllowance?.remaining ?? 20} translations left` : '$19.99 / month'}</small>
      </button>
    </div>
  );
}
